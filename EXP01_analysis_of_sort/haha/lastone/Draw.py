...
Algorithm = ['Insertion Sort','Bubble Sort', 'Quick Sort', 'Merge Sort', 'Shell Sort']
Algorithm2 = ['Quick Sort', 'Merge Sort', 'Shell Sort']
# 读取文件
...
def plot_data(scales, run_times, compare_counts, swap_counts):
    ...
    # 保存图表
    fig1.savefig('run_times.png')
    fig2.savefig('compare_counts.png')
    fig3.savefig('swap_counts.png')
...
def plot_data2(scales, run_times, compare_counts, swap_counts):
    # 创建三个图形
    fig1, ax1 = plt.subplots(figsize=(10, 5))
    fig2, ax2 = plt.subplots(figsize=(10, 5))
    fig3, ax3 = plt.subplots(figsize=(10, 5))
    ...
    # 绘制运行时间
    for i in range(3):
        ax1.scatter(scales, run_times[i], label=Algorithm2[i])
        ax2.scatter(scales, compare_counts[i], label=Algorithm2[i])
        ax3.scatter(scales, swap_counts[i], label=Algorithm2[i])
    ...
    # 保存图表
    fig1.savefig('run_times2.png')
    fig2.savefig('compare_counts2.png')
    fig3.savefig('swap_counts2.png')
...
# 主函数
def main():
    file_path = 'output.txt'  # 替换为您的txt文件路径
    scales, run_times, compare_counts, swap_counts = read_data(file_path)
    plot_data(scales, run_times, compare_counts, swap_counts)

    file_path2 = 'output2.txt'  # 替换为您的txt文件路径
    scales, run_times, compare_counts, swap_counts = read_data2(file_path2)
    plot_data2(scales, run_times, compare_counts, swap_counts)
